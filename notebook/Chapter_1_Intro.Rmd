...
base_size = 13 # so graphs are the same size
```
...
eff_power_g1 <- es_by_pwr_df |> 
  ggplot(aes(x = n, y = effect_size, colour = as.numeric(power), group = power))+
  geom_line(alpha = 0.8, size = 1.5)+
  scale_x_log10(name = "Sample Size")+
  scale_y_continuous(name = "Cohen's d")+
  guides(colour = guide_colourbar(title = "Power", barwidth = 10, barheight = .8, title.vjust = 1))+
...
ggsave(plot = eff_power_g1, filename = "ch1_crit_eff_power.png", path = here::here("images"), dpi = 320, width = 10, height = 8)
```
...
es_by_alpha_df |> 
  ggplot(aes(x = n, y = crit_eff, colour = as.numeric(alpha), group = as.numeric(alpha)))+
  geom_line(alpha = 0.8, size = 1.5)+
  scale_x_log10(name = "Sample Size")+
  scale_y_continuous(name = "Cohen's d", limits = c(0, 4))+
  scale_color_gradient(name = bquote(alpha), 
                       high = "#132B43", 
                       low = "#56B1F7",
                       trans = "log10", 
                       breaks = c(0.1, .001, 0.00001), 
                       labels = c("0.1", "0.001", "0.00001"),
...
ggsave(filename = "ch1_crit_eff_alpha.png", path = here::here("images"), dpi = 320, width = 10, height = 8)
```
