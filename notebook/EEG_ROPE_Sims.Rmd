...
.rep_dyn_p1 <- .rep_p1[[1]] + 
  geom_point(data = dyn_stats, aes(x = time, y = sig_rule1 - .8, fill = '3' ),
             alpha = line_alpha, inherit.aes = F, size = 1, shape = 22, stroke = NA)+
  geom_point(data = dyn_stats, aes(x = time, y = sig_rule2 - .6, fill = '4' ),
             alpha = line_alpha, inherit.aes = F, size = 1, shape = 22, stroke = NA)+
  ...
  labs(subtitle =  " Using Dynamic Alpha Rule - <b> Min (\u03B1, \u03B2) </b>")+
...
sim_rope_1 <- read_csv(here::here("sim_data","ch4_eeg_sims", "sim_egg_pipeline_eff.csv"))
