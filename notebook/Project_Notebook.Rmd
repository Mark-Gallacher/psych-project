...
knitr::kable(x = head(dyn_alpha, n = 10), col.names = many_cohens_d)
```

#### Changing Column Names, Adding Sample Size and Reorganising tibble for visualisations
```{r}
# Finding Power when alpha is 0.05, then find beta (1 - power), 
# and pick the smallest number, to get the new alpha
...
power_dyn_a <- get_pwr_or_eff_dyn_alpha(
  col_vec = many_cohens_d, 
  row_vec = sample_size,
...
# Matrix of power levels using the dynamic alpha matrix (f_dyn_alpha), cols represent different cohen's d
# whilst rows represent different sample sizes
knitr::kable(x = head(power_dyn_a, n = 10), col.names = many_cohens_d)
```
#### Changing Column Names, Adding Sample Size and Reorganising tibble for visualisations
```{r}
power_dyn_a_df <- tidy_output_with_n(
  array = power_dyn_a,
  value_colname = "power",
  name_colname = "effect_size",
  col_vec = many_cohens_d,
...
```{r}
cohen_dyn_a <- get_pwr_or_eff_dyn_alpha(
  col_vec = many_power, 
  row_vec = sample_size,
  alpha_vec = f_dyn_alpha,
  get_power = FALSE)
...
knitr::kable(x = head(cohen_dyn_a, n = 10), col.names = many_power)
```
#### Changing Column Names, Adding Sample Size and Reorganising tibble for visualisations
```{r}
