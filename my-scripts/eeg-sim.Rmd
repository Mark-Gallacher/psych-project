...
```{r}
p_values <- array(NA, Nt)
md_values <- array(NA, Nt) # eman difference values

for(t in 1:Nt){
  t_test_values <- t.test(cond1[,t], cond2[,t], var.equal = FALSE)
  p_values[t] <- t_test_values[["p.value"]]
  mn_values[t] <- abs(t_test_values[["estimate"]][[1]] - t_test_values[["estimate"]][[2]])
}
...
rope_noise <- quantile(mn_values[1:16]) # from 10-160 milliseconds - pre-stimulus
```

Plotting P-values
```{r}
t_test_df |> 
  ggplot(aes(x = time, y = -log(value)))+
  geom_point(aes(colour = value < (0.05/ Nt)), show.legend = F, size = 2)+
  geom_line() +
  geom_hline(aes(yintercept = -log(0.05/ Nt)))+
  theme_minimal()
...
t_test_df |> 
  ...
  geom_line()+
  geom_point(aes(colour = value < (0.05/ Nt)), show.legend = F, size = 2)+
  geom_hline(aes(yintercept = 0.05/ Nt))+
  theme_minimal()
```
...
t_test_df |> 
  ...
  geom_line()+
  geom_point(aes(colour = mn_diff > rope_noise[2]), size = 2, show.legend = F)+
  theme_minimal()+
  annotate("rect", 
           xmin = 0, 
