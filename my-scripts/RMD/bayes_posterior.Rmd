...
plot_posterior <- function(df, hdi.df, null = T){
  ...
  scale_x_continuous("Mean Difference", breaks = waiver(), limits = x_limits)+
  ...
  labs(title = "Parameter Estimation Across Different Sample Sizes", 
       subtitle = subtitle)+
  geom_text(data = text_df, aes(x = -Inf, y = 1, label = paste("N =", n)), hjust = 0, vjust = -3.5, fontface = "bold")+
...
models_df <- read_csv(here::here("sim_data", "ch3_bayes", "bayes_models_null.csv"))
...
bayes_models2 <- sim_df_1 |> 
  filter(!eval(time_cond)) |> 
  pivot_for_mn(replication = F) |> 
  group_nest(n_trial, .key = "values") |> 
  mutate(model = map(.x = values, 
    .f = ~ brms::brm(
  bf(mn ~ 0 + cond, sigma ~ cond),  
  data = .x,
  family = student, 
  chains = n_chains,
  iter = n_iter, 
...
```{r, eval = F, fig.height = 8, fig.height= 10}
bayes_models_df2 |> 
